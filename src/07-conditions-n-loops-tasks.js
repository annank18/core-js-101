...
function isInsideCircle(circle, point) {
  const max = circle.center.x + circle.center.y + circle.radius;
  const min = circle.center.x + circle.center.y - circle.radius;
  const pointSum = point.x + point.y;
  if ((max > pointSum) && (min < pointSum)) return true;
  return false;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const res = [];
  if (isStartIncluded === true) res.push('[');
  else res.push('(');
  if (a < b) res.push(a, ', ', b);
  else res.push(b, ', ', a);
  if (isEndIncluded === true) res.push(']');
  else res.push(')');
  return res.join('');
}
...
function getDigitalRoot(num) {
  let result = num;
  while (result > 10) {
    result = result.toString().split('').reduce((prev, item) => (prev + +item), 0);
  }
  return result;
}
